...
14. **新增 SOCKS4/SOCKS5 协议支持**; (2026-09-24)
   - 代理模型 `Proxy` 新增 `proto` 字段（http/socks4/socks5/multi）
   - `BaseFetcher` 新增 `default_proto` 可选属性（socks5 源需覆盖）
   - 新增 `socks5_validator` 校验链路，校验时按 `proxy.proto` 分支走不同链路
   - 数据库按协议分 hash 存储（`use_socks5_proxy` / `use_socks4_proxy`）
   - `/get` 等接口支持 `?type=socks5|socks4|http|https` 过滤
15. **SOCKS5 校验由 curl 子进程改为 requests + PySocks**（`socks5h://` 让代理解析 DNS），省去 fork 开销; (2026-09-24)
16. **https 校验重写为隧道校验 `httpsTunnelValidator`**（修正 scheme + 双目标 OR），修复池内 https 代理近乎为零的历史问题; (2026-09-24)
17. **校验目标由 httpbin/qq 改为 baidu**，消除目标站抽风 / 间歇 501 导致的误判; (2026-09-24)
18. **`/get/?check=1` 现场拨测**：返回前现场验证（最多 6 个），失败即从池中剔除; (2026-09-24)
19. **健康代理降频复验**（`STABLE_RECHECK_MINUTES=10`），校验 CPU 开销降低一个量级; (2026-09-24)
20. 新增代理源 **monosans**（http/socks4/socks5）、**roosterkid**（HTTPS 密度高）、**hookzof**（SOCKS5 大户）; (2026-09-24)
21. 扩展代理源 **Proxifly** HTTPS 协议子集过滤; (2026-09-24)
22. 新增 `/stats` 聚合统计接口（协议/来源/地域分布、最近代理、日志尾部）; (2026-09-24)
23. 新增 `/dashboard` 图形化代理面板、`/api-docs` API 文档页（自包含单文件 `web/`，无需前端构建）; (2026-09-24)
24. `/get`/`/pop`/`/all` 统一支持 `?type=http/https/socks4/socks5` 协议过滤；`/get` 新增 `?region=CN` 地域过滤（大小写不敏感）; (2026-09-24)
25. **`type=https` 语义收敛**为「http 协议 + 支持 https」（`proto=http` 且 `https=true`），排除 socks5/socks4（即使其 `https` 字段为 true）; (2026-09-24)
26. `/stats` 提供 `by_region` 地域分布统计；`/dashboard` 支持协议/地域筛选与现场拨测获取代理; (2026-09-24)
27. API 文档页样例代码一键复制，修复 HTML `&` 转义导致的 URL 参数异常; (2026-09-24)
28. gunicorn `timeout=60`：修复现场拨测（最坏 24s）超时触发 worker abort（`SystemExit`）导致 500 的问题; (2026-09-24)
