...
class _ThreadFetcher(Thread):
    ...
    def run(self):
        ...
        default_proto = getattr(self.fetcher_class, "default_proto", "http")
        self.log.info("ProxyFetch - {func}: start (proto={proto})".format(
            func=fetcher_name, proto=default_proto))
        try:
            for proxy in self.fetcher_class().fetch():
                ...
                if proxy in self.proxy_dict:
                    ...
                    # 同一代理被不同协议的 fetcher 抓到时，升级为多协议标记
                    if default_proto != self.proxy_dict[proxy].proto:
                        self.proxy_dict[proxy].proto = "multi"
                else:
                    self.proxy_dict[proxy] = Proxy(
                        proxy, source=fetcher_name, proto=default_proto)
        except Exception as e:
